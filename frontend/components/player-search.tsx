...
export function PlayerSearch({
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/v1/players?q=${encodeURIComponent(query)}`, {
        ...
        <div className="absolute left-0 top-12 z-30 w-full min-w-[20rem] rounded-lg border border-white/15 bg-[#151515] p-1 shadow-2xl">
          {results.length ? (
            results.map((player) => (
              <button
                key={player.slug}
                onMouseDown={() => selectPlayer(player.slug)}
                className="block w-full px-3 py-3 text-left hover:bg-white/[0.06]"
              >
